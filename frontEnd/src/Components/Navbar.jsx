  ...
  return (
    <div className=' flex item-center justify-between py-5 font-medium'>
     ...
     <ul className=' hidden sm:flex gap-5 text-sm text-gray-700'>
      ...
      <NavLink to='/recomended' className='flex  flex-col items-center gap-1'>
        <p>RECOMMENDED</p>
        <hr className=' w-2/4 border-none h-[1.5px] bg-gray-700 hidden'></hr>
      </NavLink>
    ...
    <div className=' flex flex-col gap-2 w-36 py-3 bg-slate-100 text-gray-500 rounded'>
     ...
     <img onClick={()=>{setvisible(true)}} src={assets.menu_icon} className=' w-5 cursor-pointer md:hidden'/>
      ...
      <div className={`absolute  top-0 right-0 bottom-0 overflow-hidden bg-white  transition-all ${visible ? 'w-full ': 'w-0'}`}>
          ...
          <NavLink onClick={()=>{setvisible(false)}} to='/recomended' className='py-2 pl-6 border'>RECOMMENDED</NavLink>
